...
void WindowsTerminalBackendTest::runsCmdWithWorkingDirectoryAndExitCode()
{
    ...
    QVERIFY(backend->start(cmdProfile({QStringLiteral("/D"),
                                       QStringLiteral("/Q"),
                                       QStringLiteral("/C"),
                                       QStringLiteral("echo ZEALRN_CONPTY & cd & exit /b 7")}),
                           directory.path(),
                           QSize(90, 30)));
...
void WindowsTerminalBackendTest::preservesPowerShellUnicode()
{
    ...
    const TerminalProfile profile = {powershell,
                                     ...
                                     {QStringLiteral("-NoLogo"),
                                      ...
                                      QStringLiteral("Write-Output '%1'").arg(expected)}};
...
void WindowsTerminalBackendTest::destructionTerminatesProcessTree()
{
    ...
    const TerminalProfile profile = {powershell,
                                     ...
                                     {QStringLiteral("-NoLogo"),
                                      QStringLiteral("-NoProfile"),
                                      QStringLiteral("-Command"),
                                      QStringLiteral("$PID | Set-Content -LiteralPath '%1'; while ($true) { Start-Sleep 1 }")
                                          .arg(QDir::toNativeSeparators(pidPath))}};
    ...
    bool ok = false;
    const DWORD processId = pidFile.readAll().trimmed().toULong(&ok);
    QVERIFY(ok);
